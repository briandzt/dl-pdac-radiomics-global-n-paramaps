...
class nnUNetTrainer(object):
    ...
    def initialize(self):
        if not self.was_initialized:
            ...
            self.network.enable_attention_from_dim(global_feat_dim=10, num_heads=8)
    ...
    def build_network_architecture(architecture_class_name: str,
        ...
        """
        print(architecture_class_name)
        for k,v in arch_init_kwargs.items():
            print(k,v)
    ...
    def train_step(self, batch: dict) -> dict:
        data = batch['data']
        target = batch['target']
        global_info = batch['global_info']
        # allow optional global_info
        if global_info is not None:
            assert isinstance(global_info, torch.Tensor), f'{type(global_info)}'
        # print(global_info.size())

        data = data.to(self.device, non_blocking=True)
        if global_info is not None:
            global_info = global_info.to(self.device, non_blocking=True)
        ...
        with autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
            output = self.network(data, global_info)
            # del data
