...
class nnUNetDataLoader2D(nnUNetDataLoaderBase):
    def generate_train_batch(self):
        ...
        for j, current_key in enumerate(selected_keys):
            ...
            # print(self._data)
            # print(current_key)
            data, seg, properties, global_info = self._data.load_case(current_key)
            # print('data_loader_2d: global_info captured')
        ...
        if self.transforms is not None:
            ...
            return {'data': data_all, 'target': seg_all, 'keys': selected_keys,
                    'global_info': (torch.tensor(global_info, dtype=torch.float) if global_info is not None else None)}
