...
def preprocess_fromfiles_save_to_queue(list_of_lists: List[List[str]],
                                       ...
                                       abort_event: Event,
                                       verbose: bool = False,
                                       global_info_folder: Union[None, str] = None):
    try:
        ...
        for idx in range(len(list_of_lists)):
            ...
            # Load global_info if folder is provided
            global_info = None
            if global_info_folder is not None and output_filenames_truncated is not None and output_filenames_truncated[idx] is not None:
                import os
                case_name = os.path.basename(output_filenames_truncated[idx])
                global_info_path = os.path.join(global_info_folder, f"{case_name}_global.npy")
                if os.path.exists(global_info_path):
                    global_info = torch.from_numpy(np.load(global_info_path)).to(dtype=torch.float32)
