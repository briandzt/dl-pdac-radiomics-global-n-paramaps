...
class nnUNetPredictor(object):
    def __init__(self,
                 ...
                 global_info_folder: str = None):
        self.verbose = verbose
        self.verbose_preprocessing = verbose_preprocessing
        self.allow_tqdm = allow_tqdm
        self.global_info_folder = global_info_folder
    ...
    def _internal_get_data_iterator_from_lists_of_filenames(self,
        ...
        return preprocessing_iterator_fromfiles(input_list_of_lists, seg_from_prev_stage_files,
                                                ...
                                                self.verbose_preprocessing, self.global_info_folder)
    ...
    def predict_from_data_iterator(self,
        ...
        with multiprocessing.get_context("spawn").Pool(num_processes_segmentation_export) as export_pool:
            ...
            for preprocessed in data_iterator:
                ...
                global_info = preprocessed.get('global_info', None)
                ...
                prediction = self.predict_logits_from_preprocessed_data(data, global_info).cpu()
    ...
    def predict_logits_from_preprocessed_data(self, data: torch.Tensor, global_info: torch.Tensor = None) -> torch.Tensor:
        """
        ...
        if global_info is not None:
            print(f'Global info shape: {global_info.shape}')
        ...
        for params in self.list_of_parameters:
            ...
            # If checkpoint has attention keys and network exposes lazy-attn, enable it before loading
            has_attn_keys = any(k.startswith('attn_layer.') for k in params.keys())
            mod = self.network._orig_mod if isinstance(self.network, OptimizedModule) else self.network
            if has_attn_keys and hasattr(mod, 'enable_attention_from_dim'):
                # try to infer global dim from checkpoint or provided global_info
                inferred_global_dim = None
                # proj.weight is [C_bottleneck, global_dim]
                if 'attn_layer.proj.weight' in params:
                    inferred_global_dim = params['attn_layer.proj.weight'].shape[1]
                elif 'attn_layer.proj.bias' in params:
                    # bias is [C_bottleneck]; cannot infer, fallback later
                    pass
                if inferred_global_dim is None and isinstance(mod, nn.Module):
                    try:
                        # last resort: if a global_info tensor was passed, use its feature dim
                        if global_info is not None:
                            inferred_global_dim = int(global_info.shape[1])
                    except Exception:
                        pass
                if inferred_global_dim is not None:
                    mod.enable_attention_from_dim(inferred_global_dim)

            # load parameters (strict by default)
            ...
            if prediction is None:
                prediction = self.predict_sliding_window_return_logits(data, global_info).to('cpu')
            else:
                prediction += self.predict_sliding_window_return_logits(data, global_info).to('cpu')
    ...
    def _internal_maybe_mirror_and_predict(self, x: torch.Tensor, global_info: torch.Tensor = None) -> torch.Tensor:
        mirror_axes = self.allowed_mirroring_axes if self.use_mirroring else None
        if global_info is not None:
            prediction = self.network(x, global_info)
        else:
        ...
        if mirror_axes is not None:
            ...
            for axes in axes_combinations:
                if global_info is not None:
                    prediction += torch.flip(self.network(torch.flip(x, axes), global_info), axes)
                else:
    ...
    def _internal_predict_sliding_window_return_logits(self,
        ...
        try:
            ...
            # Move global_info to device if provided
            if global_info is not None:
            ...
            for sl in tqdm(slicers, disable=not self.allow_tqdm):
                ...
                prediction = self._internal_maybe_mirror_and_predict(workon, global_info)[0].to(results_device)
    ...
    def predict_sliding_window_return_logits(self, input_image: torch.Tensor, global_info: torch.Tensor = None) \
            ...
            with torch.autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
                ...
                if self.perform_everything_on_device and self.device != 'cpu':
                    ...
                    try:
                        ...
                        empty_cache(self.device)
                        predicted_logits = self._internal_predict_sliding_window_return_logits(data, slicers, False, global_info)
                else:
                    predicted_logits = self._internal_predict_sliding_window_return_logits(data, slicers,
                                                                                           self.perform_everything_on_device,
                                                                                           global_info)
...
def predict_entry_point():
    ...
    parser.add_argument('--disable_progress_bar', action='store_true', required=False, default=False,
                        help='Set this flag to disable progress bar. Recommended for HPC environments (non interactive '
                             'jobs)')
    parser.add_argument('-global_info_folder', type=str, required=False, default=None,
                        help='Folder containing global_info files (*.npy) with same naming convention as input images but with _global.npy suffix')
    ...
    predictor = nnUNetPredictor(tile_step_size=args.step_size,
                                ...
                                verbose_preprocessing=args.verbose,
                                allow_tqdm=not args.disable_progress_bar,
                                global_info_folder=args.global_info_folder)
