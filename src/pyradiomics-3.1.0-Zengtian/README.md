...
In addition to the standard features included in PyRadiomics, this customized version includes the following enhancements:

### Wavelet Direction Filtering

You can now specify which wavelet decompositions to use when extracting features from wavelet-transformed images.
This is useful when you only need specific decompositions rather than all of them, which can significantly reduce
computation time and result size.

To use this feature, add the `waveletDirection` parameter to your YAML configuration file under the Wavelet imageType:

```yaml
imageType:
  Wavelet:
    waveletDirection: ['HHL']  # Only extract features from HHL decomposition
```

For 3D images, possible values are combinations of 'H' (High pass filter) and 'L' (Low pass filter) in each dimension,
such as 'HHL', 'LLH', 'HHH', etc. For 2D images, only two letters are used, e.g., 'HL', 'LH', 'HH', 'LL'.

See the example file `examples/exampleSettings/HHLonly_wavelet.yaml` for a complete example.
