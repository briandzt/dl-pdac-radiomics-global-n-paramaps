...
def getWaveletImage(inputImage, inputMask, **kwargs):
  ...
  - waveletDirection [None]: List of strings, specifies which wavelet decomposition directions to use.
    If None, all decompositions are used (default). Available directions depend on dimensionality, 
    e.g., ['HHH', 'HHL', 'HLH', 'HLL', 'LHH', 'LHL', 'LLH'] for 3D images.
  ...
  # Get the wavelet directions to process
  waveletDirections = kwargs.get('waveletDirection', None)
  if waveletDirections is not None:
    logger.debug('Using specified wavelet directions: %s', waveletDirections)

  for idx, wl in enumerate(ret, start=1):
    for decompositionName, decompositionImage in wl.items():
      # Skip decompositions not in waveletDirections if specified
      if waveletDirections is not None and decompositionName not in waveletDirections:
        logger.debug('Skipping wavelet %s (not in specified directions)', decompositionName)
        continue
  ...
  # Always yield the approximation image
  if len(ret) == 1:
    inputImageName = 'wavelet-%s' % ('L' * len(axes))
  else:
    inputImageName = 'wavelet%s-%s' % (len(ret), ('L' * len(axes)))
  
  # Check if the approximation (all 'L's) should be yielded
  approximationName = 'L' * len(axes)
  if waveletDirections is None or approximationName in waveletDirections:
    logger.debug('Yielding approximation (%s) image', inputImageName)
    yield approx, inputImageName, kwargs
  else:
    logger.debug('Skipping approximation (%s) (not in specified directions)', inputImageName)
